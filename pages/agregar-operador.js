...
const reporte = () => {
    ...
    const fetcher = () => axios('/api/operador').then(datos => datos.data)
        ...
        <LayoutPalet pagina={`Operadores`}>
            <form onSubmit={AgregarOperador}>
                <div className="flex items-center justify-center p-2 space-x-2 bg-white">
                    ...
                    <div className={`bg-red-600 py-3 px-5 text-white font-semibold rounded-lg hover:shadow-lg transition duration-3000 cursor-pointer ${comprobarPedido() ? '' : 'opacity-50 cursor-not-allowed'}`}>
                        <input
                            type="submit"
                            value="Agregar"
            ...
            <div className='mx-auto w-full max-w-2xl  border-gray-200 bg-white'>
                <header class="border-b border-gray-100 px-5 py-4">
                    <div class="font-semibold text-gray-800">Listado de Operadores</div>
                </header>
            </div>
            <p className="text-2xl my-5"></p>
            <div className=''>
                {data && data.length ? data.map(operadores =>
                <div key={operadores.id} className=''>
                    <ListadoOperadores
                        operadores={operadores}
                    />
                </div>
                ): <div className='text-center pb-1'>Cargado... Operadores<Spinner className='flex m-auto' /></div>
                }
            </div>  
        </LayoutPalet>
    </>
