...
const ListadoPdfOcGeneral = ({despacho}) => {
    const {id,pedido01, fecha,cliente, destino} = despacho
  ...
  return (
    ...
    <>
        <div className="shadow-lg rounded-lg p-2">
            <div>
                {pedido01.map(oc => (
                    <div className="grid grid-cols-2" key={oc.id}>
                        <div className="m-auto text-center">
                            <h1 className=" font-semibold">Nº: {id}</h1>
                            <p className=" font-semibold">{formatiarFecha(fecha)}</p>
                            <p className=" font-semibold">{cliente}</p>
                            <p className=" font-semibold">{destino}</p>
                        </div>
                        <div>
                        {
                                <Link href={`/despachos/${id}`} className="flex justify-center">
