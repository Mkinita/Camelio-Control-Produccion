...
const OrdenGeneral = ({largos}) => {
    const {id,largo} = largos
    ...
    const eliminarRegistro = async () => {
        const confirmarCreacion = window.confirm(
            `¿Estás seguro de que deseas eliminar el largo ${largo}?`
          );
        
          if (confirmarCreacion) {
        try {
            const response = await fetch(`/api/eliminarlargo/${id}`, {
                method: 'DELETE',
            });

            if (response.ok) {
                toast.success(`${largo} eliminada`);
                setTimeout(() => {
    ...
    function reloadPage() {
        toast.success(`${largo} Actualizando`);
            setTimeout(() => {
            router.push('/agregar-largo');
        }, 1000);
    }
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          await axios.post(`/api/editarlargo/${id}`, 
          { largo: newlargo
          });
          setNewLargo(newlargo);
