...
const OrdenGeneral = ({pallets}) => {
    const {id,pallet} = pallets
    ...
    const eliminarRegistro = async () => {
        try {
        const response = await fetch(`/api/eliminarPallet/${id}`, {
            method: 'DELETE',
        });

        if (response.ok) {
            toast.success(`${pallet} eliminada`);
            setTimeout(() => {
    ...
    function reloadPage() {
        toast.success(`${pallet} Actualizando`);
            setTimeout(() => {
            router.push('/agregar-producto-palet');
        }, 1000);
    }
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          await axios.post(`/api/editarpallet/${id}`, 
          { pallet: newpallet
          });
          setNewPallet(newpallet);
                    ...
                    <div class="overflow-x-auto p-3">
                        <table class="w-full table-auto">
                            ...
                            <tbody class="divide-y divide-gray-100 text-sm">
                                <tr>
                                    <td class="p-2">
                                        <div class="font-medium text-gray-800">{pallet}</div>
                                    </td>
                                    <td className="p-2">
                                                ...
                                                {isVisible ? (
                                                    <div className="items-center flex-1">
                                                        <input
                                                            ...
                                                            type="text"
                                                            value={newpallet}
                                                            onChange={(event) => setNewPallet(event.target.value)}
                                                        />
                                                    </div>
                                                    ) : (
