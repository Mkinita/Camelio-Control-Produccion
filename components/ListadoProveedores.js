...
const OrdenGeneral = ({proveedores}) => {
    const {id,proveedor} = proveedores
    ...
    const eliminarRegistro = async () => {
        try {
        const response = await fetch(`/api/eliminarproveedor/${id}`, {
            method: 'DELETE',
        });

        if (response.ok) {
            toast.success(`${proveedor} eliminada`);
            setTimeout(() => {
    ...
    function reloadPage() {
        toast.success(`${proveedor} Actualizando`);
            setTimeout(() => {
            router.push('/agregar-valores');
        }, 1000);
    }
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          await axios.post(`/api/editarprveedor/${id}`, 
          { proveedor: newproveedores
          });
          setNewProveedores(newproveedores);
                                                ...
                                                {isVisible ? (
                                                    <div className="items-center flex-1">
                                                        <input
                                                            ...
                                                            onChange={(event) => setNewProveedores(event.target.value)}
                                                        />
                                                    </div>
                                                    ) : (
