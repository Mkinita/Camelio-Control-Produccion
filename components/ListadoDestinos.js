...
const OrdenGeneral = ({destinos}) => {
    const {id,destino} = destinos
    ...
    const eliminarRegistro = async () => {
        const confirmarCreacion = window.confirm(
            `¿Estás seguro de que deseas eliminar el destino ${destino}?`
          );
        
          if (confirmarCreacion) {
        try {
            const response = await fetch(`/api/eliminardestino/${id}`, {
                method: 'DELETE',
            });

            if (response.ok) {
                toast.success(`${destino} eliminada`);
                setTimeout(() => {
    ...
    function reloadPage() {
        toast.success(`${destino} Actualizando`);
            setTimeout(() => {
            router.push('/agregar-destino');
        }, 1000);
    }
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          await axios.post(`/api/editardestino/${id}`, 
          { destino: newdestino
          });
          setNewDestino(newdestino);
