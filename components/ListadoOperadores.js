...
const OrdenGeneral = ({operadores}) => {
    const {id,operador} = operadores
    ...
    const eliminarRegistro = async () => {
        const confirmarCreacion = window.confirm(
            `¿Estás seguro de que deseas eliminar el operador ${operador}?`
          );
        
          if (confirmarCreacion) {
        try {
            const response = await fetch(`/api/eliminaroperador/${id}`, {
                method: 'DELETE',
            });

            if (response.ok) {
                toast.success(`${operador} eliminada`);
                setTimeout(() => {
    ...
    function reloadPage() {
        toast.success(`${operador} Actualizando`);
            setTimeout(() => {
            router.push('/agregar-operador');
        }, 1000);
    }
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          await axios.post(`/api/editaroperador/${id}`, 
          { operador: newoperador
          });
          setNewOperador(newoperador);
                    ...
                    <div class="overflow-x-auto p-3">
                        <table class="w-full table-auto">
                            ...
                            <tbody class="divide-y divide-gray-100 text-sm">
                                <tr>
                                    <td class="p-2">
                                        <div class="font-medium text-gray-800">{operador}</div>
                                    </td>
                                    <td className="p-2">
                                                ...
                                                {isVisible ? (
                                                    <div className="items-center flex-1">
                                                        <input
                                                            ...
                                                            type="text"
                                                            value={newoperador}
                                                            onChange={(event) => setNewOperador(event.target.value)}
                                                        />
                                                    </div>
                                                    ) : (
