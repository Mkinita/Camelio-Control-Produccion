...
const OrdenGeneral = ({calidades}) => {
    const {id,calidad} = calidades
    ...
    const eliminarRegistro = async () => {
        try {
        const response = await fetch(`/api/eliminarcalidad/${id}`, {
            method: 'DELETE',
        });
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          await axios.post(`/api/editarcalidad/${id}`, 
          { calidad: newcalidad
          });
          setNewCalidad(newcalidad);
                                                ...
                                                {isVisible ? (
                                                    <div className="items-center flex-1">
                                                        <input
                                                            ...
                                                            type="text"
                                                            value={newcalidad}
                                                            onChange={(event) => setNewCalidad(event.target.value)}
                                                        />
                                                    </div>
                                                    ) : (
