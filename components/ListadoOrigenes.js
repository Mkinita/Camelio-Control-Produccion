...
const OrdenGeneral = ({origenes}) => {
    const {id,origen} = origenes

    const [neworigenes, setNewOrigenes] = useState('');
    ...
    const eliminarRegistro = async () => {
        try {
        const response = await fetch(`/api/eliminarorigen/${id}`, {
            method: 'DELETE',
        });

        if (response.ok) {
            toast.success(`${origen} eliminada`);
            setTimeout(() => {
    ...
    function reloadPage() {
        toast.success(`${origen} Actualizando`);
            setTimeout(() => {
            router.push('/agregar-origen');
        }, 1000);
    }
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          await axios.post(`/api/editarorigen/${id}`, 
          { origen: neworigenes
          });
          setNewOrigenes(neworigenes);
                                                ...
                                                {isVisible ? (
                                                    <div className="items-center flex-1">
                                                        <input
                                                            ...
                                                            type="text"
                                                            value={neworigenes}
                                                            onChange={(event) => setNewOrigenes(event.target.value)}
                                                        />
                                                    </div>
                                                    ) : (
